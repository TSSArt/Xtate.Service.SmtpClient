...
{
	private static readonly ConcurrentDictionary<Type, IStub> Instances = new(concurrencyLevel: 1, capacity: 4);
	...
	private static IStub GetStubInstance(Type stubType) => stubType == typeof(Any) ? Any : Instances.GetOrAdd(stubType, static t => (IStub) Activator.CreateInstance(t)!);

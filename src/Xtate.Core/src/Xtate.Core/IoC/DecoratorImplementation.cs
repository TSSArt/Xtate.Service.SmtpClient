...
public readonly struct DecoratorImplementation<TImplementation, TArg> where TImplementation : notnull
{
	...
	public DecoratorImplementation(IServiceCollection serviceCollection, InstanceScope instanceScope)
	{
	...
	{
		var decorator = DecoratorFactoryProvider<TImplementation, TService, TArg>.Delegate;
		_serviceCollection.Add(new ServiceEntry(TypeKey.ServiceKey<TService, TArg>(), _instanceScope, decorator));
	}
}